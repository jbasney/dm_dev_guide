...
Instead you must enable the `Red Hat Developer Toolset`_ version 6 (``devtoolset-6``) which has been pre-installed.
This provides an updated toolchain, including GCC 6.2.1.

This version of GCC is also used for building the :ref:`shared stacks <lsst-dev-loadlsst>` available on the machine and must be used if building against those stacks.
It is also used by the ``lsstsw`` account that builds releases.

Enable and test ``devtoolset-6`` using the ``scl`` command as follows (replacing ``bash`` with your shell of choice if necessary):
...
The Developer Toolset includes version 1.9.4 of the `Git`_ version control system.

You may wish to automatically enable ``devtoolset-6`` every time you log in to ``lsst-dev01`` by adding it to your shell initialization files.
